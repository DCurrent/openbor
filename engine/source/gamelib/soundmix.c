...
void sound_update_adpcm(){
	...
	if((adpcm_handle<0) || (music_type!=0)) return;
...
int sound_query_ogg(char *artist, char *title){
	...
	if (!artist || !title) return 1;

	for(i=0; i<comment->comments; i++)
	{
		current = comment->user_comments[i];
		if (strncmp("ARTIST=", current, 7) == 0)      strcpy(artist, current+7);
		else if (strncmp("TITLE=", current, 6) == 0)  strcpy(title, current+6);
	}
...
int sound_open_music(char *filename, char *packname, int volume, int loop, u32 music_offset){
	...
	if(sound_open_adpcm(fnam, packname, volume, loop, music_offset)) return 1;
	sprintf(fnam, "%s.ogg", filename);
	if(sound_open_ogg(fnam, packname, volume, loop, music_offset)) return 1;
...
void sound_close_music(){
	switch(music_type){
		case 0: sound_close_adpcm(); break;
		case 1: sound_close_ogg();
	}
	music_type = -1;
}
...
void sound_update_music(){
	switch(music_type){
		case 0: sound_update_adpcm(); break;
		case 1: sound_update_ogg();
	}
}

int sound_query_music(char *artist, char *title){
	switch(music_type){
		case 0:  return sound_query_adpcm(artist, title);
		case 1:  return sound_query_ogg(artist, title);
		default: return 0;
	}
}
...
void sound_music_tempo(int music_tempo){
	...
	switch(music_type){
		case 0: sound_adpcm_tempo(music_tempo); break;
		case 1: sound_ogg_tempo(music_tempo);
	}
}
