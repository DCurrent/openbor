...
{
	if(node->info) free(node->info);
	if(node->left) pp_expr_destroy(node->left);
	if(node->right) pp_expr_destroy(node->right);
	free(node);
}
...
error:
	if(root->info) pp_expr_destroy(root);
	if(leftleaf) pp_expr_destroy(leftleaf);
